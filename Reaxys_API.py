...
class Reaxys_API:
    ...
    def __init__(self, proxy=None, port=None):
        ...
        self.proxy = proxy
        self.port = port
        self.ReaxysAPIjson = 'ReaxysAPI.json'
    ...
    def OpenNewSession(self, url, url_main, username, password, callername):
        self.connect(url, url_main, username, password, callername)
        ...
        cache.write(apiJson)
        cache.close()
        #print("New %s file was created" % self.ReaxysAPIjson)

    def Load(self, url, url_main, username, password, callername):
        if self.ReadFromJSON() == FileNotFoundError:
            self.OpenNewSession(url, url_main, username, password, callername)
        elif self.SessionExists() == False:
            self.OpenNewSession(url, url_main, username, password, callername)

    def GetCompoundProps(self, CompoundIDs:list, CompIdType:str, Fields:list, APIlogin:dict):
        FieldToValues = dict()
        #move next line to the function that calls this one for speed
        #self.Load(APIlogin['ReaxysURL'],'',APIlogin['ReaxysUsername'],APIlogin['ReaxysPassowrd'],APIlogin['ReaxysAPIkey'])
